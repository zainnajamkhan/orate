...
public extension Color {
    ...
    /// harder to read.
    static let orate = Color(nsColor: NSColor(name: nil) { appearance in
    ...
    static let orateRecording = Color(nsColor: NSColor(name: nil) { appearance in
        appearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
    ...
    static let orateCanvas = Color(nsColor: .windowBackgroundColor)

    /// Content that should feel like paper on top of the canvas: the transcript, the history.
    static let orateSurface = Color(nsColor: .textBackgroundColor)

    /// A quiet fill for grouped controls and empty tracks.
    static let orateWell = Color.primary.opacity(0.05)

    /// Hairlines. Deliberately low contrast: separators should be felt, not read.
    static let orateEdge = Color.primary.opacity(0.09)
}
    ...
    public func body(content: Content) -> some View {
        content
            ...
            .overlay(alignment: .bottom) {
                Rectangle().fill(Color.orateEdge).frame(height: 1)
            }
    }
}
...
public struct Well: ViewModifier {
    ...
    public func body(content: Content) -> some View {
        content
            ...
            .background(RoundedRectangle(cornerRadius: radius, style: .continuous).fill(Color.orateWell))
    }
}
        ...
        content
            ...
            .background(
                RoundedRectangle(cornerRadius: radius, style: .continuous)
                    .fill(Color.orateSurface)
            )
            .overlay(
                RoundedRectangle(cornerRadius: radius, style: .continuous)
                    .stroke(Color.orateEdge, lineWidth: 1)
            )
    }
}
...
///
/// All of them are built from `Color.orate` rather than from fresh literals, so retinting
...
public enum Brand {
    ...
    public static var accent: LinearGradient {
        LinearGradient(
            colors: [Color.orate, Color.orate.opacity(0.72)],
            startPoint: .top,
            endPoint: .bottom
        )
    }
    ...
    public static var live: LinearGradient {
        LinearGradient(
            colors: [Color.orateRecording, Color.orateRecording.opacity(0.55)],
            startPoint: .top,
            endPoint: .bottom
        )
    }
