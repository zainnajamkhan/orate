...
struct HotkeyStep: View {
                                ...
                                KeycapRow(candidate, scale: 0.9)
                                    ...
                                    .background {
                                        RoundedRectangle(cornerRadius: Radius.medium, style: .continuous)
                                            .fill(isChosen ? Color.orate.opacity(0.10) : Color.orateWell)
                                    }
                                    .overlay {
                                        RoundedRectangle(cornerRadius: Radius.medium, style: .continuous)
                                            .strokeBorder(
                                                isChosen ? Color.orate.opacity(0.55) : Color.clear,
                                                lineWidth: 1.5
                                            )
                                    }
                            }
    ...
    /// press before Orate sees it, and the registration reports no error at all. Nothing
    ...
    private var proof: some View {
        HStack(spacing: Space.medium) {
            Image(systemName: wasHeard ? "checkmark.circle.fill" : "hand.tap")
                .font(Type.lead)
                .foregroundStyle(wasHeard ? Color.green : Color.orate)
                .frame(width: 26)

            VStack(alignment: .leading, spacing: Space.hair) {
                Text(wasHeard ? "Orate heard it" : "Press it now to check")
                    .font(Type.heading)
                Text(wasHeard
                    ? "The shortcut is yours. Nothing else on this Mac is taking it first."
                    : "Some apps take a shortcut before Orate can see it. Launchers like Raycast and Alfred are the usual ones. Press yours and this will confirm it arrived.")
                    .font(Type.detail)
                    .foregroundStyle(.secondary)
